...
public class DownloadCommandHandler implements CommandHandler {
    ...
    private boolean updateStatus(String bitfield){
        Integer port = 2006;
...
//            System.out.println(response.isSuccessful());
    ...
    public void downloadRequest(int port){
        String address = "127.0.0.1";
    ...
    private void handleResult(Map<String, Object> resultMap, String outputFileName) {
        ...
        for (int pieceIdx = 0; pieceIdx < torrentStateModel.getNumPieces(); pieceIdx++) {
            ...
            if(torrentStateModel.getPeers().size() > 1){
                while(randomNumber == previousNumber){
                    randomNumber = random.nextInt(torrentStateModel.getPeers().size());
                }
            }
            if(pieceIdx <= 10 && randomNumber == 2){
                randomNumber = 0;
            }
